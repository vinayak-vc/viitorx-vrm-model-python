...
class BlazeposeDepthai:
    ...
    def __init__(self, input_src="rgb",
        ...
        # OV9782-native color mode (full-FOV). geaxgx's default geometry assumes a 1920x1080 IMX378 sensor;
        # on an OV9782 (1280x800) that mismatch makes setIspScale invalid → ISP scaling disabled → DepthAI
        # centre-CROPS the sensor → a very narrow view. When color_res is "800p"/"720p" we instead size the
        # frame from the ACTUAL sensor resolution with a valid ISP scale, preserving the full lens FOV.
        ...
        if input_src == None or input_src == "rgb" or input_src == "rgb_laconic":
            ...
            elif self.color_res in ("800p", "720p"):
                # Full-FOV path for OV9782 colour sensors (native 800p=1280x800, 720p=1280x720). Downscale
                # the FULL sensor by a valid ISP ratio (default 1/2) so the whole lens FOV is preserved,
                # instead of geaxgx's 1080p-derived ratio which the OV9782 ISP rejects → crop.
                native_w = 1280
                native_h = 800 if self.color_res == "800p" else 720
                if self.color_scale is None:
                    self.scale_nd = (1, 2)
                else:
                    self.scale_nd = self.color_scale
                self.img_w = int(round(native_w * self.scale_nd[0] / self.scale_nd[1]))
                self.img_h = int(round(native_h * self.scale_nd[0] / self.scale_nd[1]))
    ...
    def create_pipeline(self):
        ...
        if self.color_res == "800p":
            cam.setResolution(dai.ColorCameraProperties.SensorResolution.THE_800_P)
        elif self.color_res == "720p":
