...
# ---- shared capture-log loading -----------------------------------------------------------------
# Lifted here from f16_consolidate and f16_edge_check, which carried byte-identical copies. The
# confidence floor is the analysis gate used across F-16: a row is only usable when BOTH mono
# confidences clear it and both marker positions are present.
CONF_FLOOR = 0.30


def load_rows(pattern, conf=CONF_FLOOR):
    """Reads every JSONL capture matching `pattern` and returns the rows fit to analyse.

    Malformed lines are skipped rather than raising: a capture truncated by a killed run is normal
    and its complete rows are still valid evidence.
    """
    rows = []
    for path in sorted(glob.glob(pattern)):
        for line in io.open(path, encoding="utf-8"):
            try:
                ...
                continue
            if min(row.get("cL", 0), row.get("cR", 0)) < conf:
                continue
            if not (row.get("mL") and row.get("mR")):
                continue
            rows.append(row)
    return rows
